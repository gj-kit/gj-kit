...
> **2026-08-24 추기(소유자 결정) — 첫 행의 부분 개정.** 위 표의 "arbitrary-anchor overlay 합성·`asChild`" 유보 중 **트리거 슬롯과 네이티브 anchored 표면**이 memorylog2 모바일 도입 백로그 §1(SortDropdown — 앱 소유 트리거 시각, 딤 없는 anchored 패널)을 근거로 라운드 E2에서 착륙했다. 유보를 통째로 뒤집은 것이 아니라 다음 형태로 좁혀 공개했다:
> - `Menu`/`Select`의 `renderTrigger?: (trigger: TriggerRenderProps) => ReactElement` — `asChild` 합성이 아니라 **주입 props 계약**이다. 킷이 동작·접근성 전부(`ref`·`onPress`·`disabled`·role·`accessibilityState`/`aria-expanded` 병기·웹 `aria-haspopup`/`aria-controls` id·키보드/blur 핸들러·`testID`·anchored 재측정 `onLayout`)를 주입하고 소비자는 시각만 소유한다. 호스트는 press를 실제로 전달하는 Pressable(동급 pressability 래퍼)이어야 한다 — 일반 View는 주입 onPress를 조용히 버리므로 계약 밖이고, 웹 트리거의 Enter/Space 활성화는 Pressable 키 에뮬레이션에 기대지 않고 주입 키보드 핸들러가 직접 소유한다. 오용 강제는 2단계다(E2 리뷰 반영): 주입 `ref`가 open 전에 붙지 않으면 open 전이 시점의 effect에서 양 플랫폼 throw하고, 웹은 같은 effect에서 ref가 붙은 DOM 노드에 주입 배선(role·aria-expanded·접근 가능한 이름)이 실제로 도달했는지까지 검증해 부분 spread·래퍼 파킹도 throw한다 — 네이티브는 RN 호스트 props의 런타임 introspection이 불가해 ref 부착이 강제 상한이며, 네이티브 전용 부분 spread는 웹 렌더를 지나야 잡힌다. `renderTrigger`와 owned 트리거 시각 prop 조합은 타입 유니언(컴파일)과 렌더 전 `TypeError`(JS 호출자)로 이중 거부.
> - 네이티브 `presentation: 'anchored'` — 투명 Modal 안에서 `measureInWindow`로 잰 트리거 프레임에 패널을 붙인다. 웹과 같은 `placement`/`sideOffset`/`alignOffset`/`collisionPadding` 어휘를 순수 함수 `computeAnchoredPanelFrame`(내부, `computeOverlayPosition` 위임)으로 공유하고, 백드롭은 아무것도 칠하지 않되 바깥 제스처를 소유한다(밑 콘텐츠 스크롤 시도 = dismiss — reposition이 아닌 dismiss-on-scroll 채택·문서화). Escape/Back/접근성 escape는 시트와 동일하고, 재측정된 트리거가 collision boundary를 벗어나면 웹 패리티로 `anchor-detached` 사유로 닫는다(dismissDisabled는 이 정리도 보류). Android에서는 Modal에 `statusBarTranslucent`/`navigationBarTranslucent`를 켜 Modal 창 좌표계를 `measureInWindow` 좌표계와 일치시킨다(§12.11). `auto` 해석·bottom/center 경로는 불변.
> - public Host/Portal·custom anchor ref·drag/snap/grabber는 **여전히 유보** — 이 추기는 트리거 슬롯 + anchored 표면까지만 연다.
...
11. **anchored Modal의 Android 좌표계** (E2 리뷰 반영, 2026-08-24) — anchored 표면은 Dialog에 새로 연 `statusBarTranslucent`/`navigationBarTranslucent`로 Modal 창을 전체 화면으로 확장해 `measureInWindow` 좌표계와 맞춘다. jsdom unit(RNW)은 Android Modal 창 자체를 모델링하지 못하므로 edge-to-edge on/off × RN 버전 조합의 실기기 검증이 남아 있다(`navigationBarTranslucent`는 RN이 지원하는 버전에서만 적용, 미지원 버전은 무해하게 무시). memorylog2 이관 시 기기 QA 항목으로 승계.
