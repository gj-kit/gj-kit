#!/usr/bin/env node
/**
 * Verify the immutable source stamp from an expo-ui build directory or packed
 * npm tarball. It is package-owned: a consuming app may record a tarball SHA,
 * but cannot substitute that hash for provenance embedded in the artifact.
 */
...
if (sourceManifest.name !== '@gj-kit/expo-ui' || typeof sourceManifest.version !== 'string') {
  throw new Error('expo-ui package identity is invalid; refusing to verify provenance.');
}
