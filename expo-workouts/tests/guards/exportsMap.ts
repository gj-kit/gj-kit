// package.json `exports` 조건 해석 + dist 그래프의 외부 지정자 추출 (설계 §2.2 · §2.3 · §9.3).
//
// 왜 조건 **3세트**인가 — ESM/CJS 2세트만으로는 SSR 누수를 잡지 못한다. `browser` 조건만 두고
// `node`를 빠뜨리면 `web.output:"static"|"server"` 소비자의 SSR 번들이 네이티브 포크를 끌어와
// 빌드 실패 또는 하이드레이션 불일치가 된다. 그 사고는 클라이언트 번들만 보는 검사로는 영원히
// 보이지 않는다(expo-media §8.2 케이스 H).
//
// ⚠ 이 패키지는 `"type": "module"`이 **없다**(설계 §2.4-A). 따라서 ESM 산출물이 `.mjs`이고
//   CJS가 `.js`다 — expo-media와 반대다.
 ...
 * 떨어진다. 그것이 이 패키지의 의도다(설계 §2.3 규칙 5): 포크 라우팅은 `node`/`browser` 두 조건만
 * 쓰고, 네이티브는 "조건이 붙지 않은 기본 경로"를 받는다.
 */
 ...
 *
 * ⚠ `types`는 활성 조건이 아니다. 런타임 해석에 섞으면 `.d.ts`가 실행 대상으로 잡혀 그래프가
 *   통째로 어긋난다.
 */
export function resolveExport(value: ExportsValue, conditions: readonly string[]): string | null {
...
export function externalSpecifiers(entryFile: string): readonly string[] {
  ...
  while (queue.length > 0) {
    ...
    for (const ref of moduleRefs(sf)) {
      if (isRelative(ref.specifier)) queue.push(resolve(dirname(file), ref.specifier));
      else externals.add(ref.specifier);
    }
  }
