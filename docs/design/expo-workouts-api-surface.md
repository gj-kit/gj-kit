...
| 21 | **T9 · T10-iOS** | **미실행** | T9는 §2.4-A(`type:module` 회피)로 위험을 없애고, §9.3 `t9-plugin-loader`가 `pnpm test`에서, §10.2가 packed consumer에서 **두 번** 검증한다. T10-iOS(HealthKit 시트 Maestro 덤프)는 idx f51의 라벨(`Turn On All` / `Allow`)이 26.5에서 옳다고 확인됐으므로(f126) 그것으로 시작하고 첫 실기 세션에서 덤프한다 |
...
| 24 | **per-type `WRITE_*` 권한 문자열 5종** | **미검증** — Phase 0가 개별적으로 `pm grant`한 적이 없다 | `WRITE_DISTANCE`·`WRITE_ACTIVE_CALORIES_BURNED`·`WRITE_ELEVATION_GAINED`·`WRITE_HEART_RATE`·`WRITE_STEPS`는 Health Connect의 문서화된 명명 규칙에서 **파생**했다(§8.8). 소유자 결정 ②의 쓰기 경로 전체가 이 문자열들에 걸려 있고, §8.5-0의 사전 검사가 **틀린 이름으로 통과시키면** 트랜잭션이 SecurityException으로 죽는다. 완화: plugin-introspect 스냅샷이 방출 집합을 고정하고 §9.5 기기 게이트에 문자열당 `adb shell pm grant` 한 줄을 추가한다 — 몇 초의 작업이다 `[unverified]` |
