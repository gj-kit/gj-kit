 ...
 *
 * format판과 다른 점:
 * - 공개 서브패스가 4개다 → paths 매핑도 4개(`.`·`/core`·`/expo`·`/testing`).
 ...
 * - `@gj-kit/nest-operations-jobs`·`@nestjs/schedule`·`expo-server-sdk`·`@prisma/client`는
 *   devDependency가 **아니다**. README 레시피는 그것들을 import하지 않고 `declare`로 형태만
 *   세우며(설계 §5.6), 그래서 이 스크립트는 그 패키지들 없이 EXIT 0으로 통과해야 한다.
 *
...
// ── 라이브러리 유래 (실타입) ──
declare const appRelayStore: import('@gj-kit/nest-notifications/core').NotificationRelayStore;
declare const appDeliveryStore: import('@gj-kit/nest-notifications/core').NotificationDeliveryStore;
declare const appEndpointStore: import('@gj-kit/nest-notifications/core').NotificationEndpointStore;
declare const appPolicy: import('@gj-kit/nest-notifications/core').NotificationSchedulingPolicy;
declare const appPresenter: import('@gj-kit/nest-notifications/core').NotificationPresenter;
declare const appGateway: import('@gj-kit/nest-notifications/core').NotificationPushGateway;
declare const appSuite: import('@gj-kit/nest-notifications/testing').NotificationStoreSuite;
`,
);

// ── 4. tsconfig — 라이브러리 import를 dist d.ts에 매핑 (공개 엔트리 1개) ──────
const paths = {
  '@gj-kit/nest-notifications': [join(distDir, 'index.d.ts')],
  '@gj-kit/nest-notifications/core': [join(distDir, 'core.d.ts')],
  '@gj-kit/nest-notifications/expo': [join(distDir, 'expo.d.ts')],
  '@gj-kit/nest-notifications/testing': [join(distDir, 'testing.d.ts')],
};
